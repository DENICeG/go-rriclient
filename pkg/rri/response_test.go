...
func TestResponseEntity(t *testing.T) {
	...
	assert.Equal(t, ResponseEntityNameHolder, entities[0].Name())
...
func TestResponseEntityMultiHolder(t *testing.T) {
	...
	require.Len(t, response.Fields(), 12)
	assert.Equal(t, []string{string(ResultSuccess)}, response.Field(ResponseFieldNameResult))
	assert.Equal(t, []string{"8792891a-c366-11eb-bca6-bbfdc472082a"}, response.Field(ResponseFieldNameSTID))
	...
	assert.Equal(t, ResponseEntityNameHolder, entities[1].Name())
...
func TestParseBusinessMessageKV(t *testing.T) {
	...
	_, err = ParseBusinessMessageKV("Request was processed in test environment - not valid in real world [testing platform]")
	assert.Error(t, err)

	_, err = ParseBusinessMessageKV("")
	assert.Error(t, err)
}
